...
def load_test_data(image_path, fine_size_h=256, fine_size_w=144):
    img = imread(image_path)
    img = scipy.misc.imresize(img, [fine_size_h, fine_size_w])
    img = img/127.5 - 1
    return img

def load_train_data(image_path, load_size=286, fine_size_h=256, fine_size_w=144, is_testing=False):
    img_A = imread(image_path[0])
    img_B = imread(image_path[1])
    if not is_testing:
        img_A = scipy.misc.imresize(img_A, [load_size, load_size])
        img_B = scipy.misc.imresize(img_B, [load_size, load_size])
        h1 = int(np.ceil(np.random.uniform(1e-2, load_size-fine_size_h)))
        w1 = int(np.ceil(np.random.uniform(1e-2, load_size-fine_size_w)))
...
def create_image(image, size):
    return merge(inverse_transform(image), size)
